 ...
 * Shows the logged-in teacher's schedule and supports export/print actions.
 */
...
$teacher_id = (int) ($_SESSION['linked_id'] ?? 0);
if ($teacher_id <= 0) {
    $stmt = $conn->prepare("SELECT linked_id FROM users WHERE id=?");
    $stmt->bind_param("i", $_SESSION['user_id']);
    $stmt->execute();
    $teacher_id = (int) ($stmt->get_result()->fetch_assoc()['linked_id'] ?? 0);
    $_SESSION['linked_id'] = $teacher_id ?: null;
    $stmt->close();
}
...
/* ================= REUSABLE FUNCTION ================= */
function getTimetable($conn, $version_id, $teacher_id) {
    ...
    $stmt->bind_param("ii", $version_id, $teacher_id);
    $stmt->execute();
...
handleTableExport([
    ...
    'rows' => fn() => getTimetable($conn, $version_id, $teacher_id),
    'print_param' => 'print',
]);

/* ================= MAIN DATA ================= */
$result = $teacher_id > 0 ? getTimetable($conn, $version_id, $teacher_id) : false;
$rows = [];
$uniqueDays = [];
$uniqueSubjects = [];
$uniqueRooms = [];
    ...
    <section class="bg-emerald-900 border border-emerald-800 rounded-xl shadow-2xl p-8 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
        ...
        <div>
            ...
            <p class="text-emerald-100 mt-3 max-w-2xl text-sm md:text-base">
                View your assigned teaching slots, rooms, subjects, and class timings in one organized schedule.
            </p>
        </div>
    ...
    <section class="bg-white border border-slate-200 rounded-xl shadow-xl mt-6 overflow-hidden">
        ...
        <?php if ($teacher_id <= 0): ?>
            ...
            <div class="p-10 text-center text-slate-500">
                Your teacher account is not linked yet. Please contact the administrator.
            </div>

        <?php elseif (empty($rows)): ?>

            <div class="p-10 text-center text-slate-500">
                No timetable entries are assigned to you yet.
            </div>
